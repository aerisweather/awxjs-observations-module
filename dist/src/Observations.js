...
function (_super) {
  ...
  Observations.prototype.onInit = function () {
    ...
    var props = ['temps', 'feelslike', 'winds', 'dewpt', 'humidity', 'precip', 'sky'].map(function (type) {
      return "ob." + getObsProp(type) + ",ob." + getObsProp(type, true);
    });
    var request = this.account.api().endpoint('observations').action("within"
    /* WITHIN */
    ...
    }).on('change:units', function (e) {// console.log('app units changed', e.data);
    });
